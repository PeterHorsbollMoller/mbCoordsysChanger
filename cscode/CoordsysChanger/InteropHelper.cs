...
namespace CoordsysChanger
{
	static class InteropHelper
	{
        ...
        {
            InteropHelper.PrintMessage(@statement);
        ...
        private static Double GetWindowHeight(int windowId)
        {
            // make sure the front window is a mapper
            string expr = string.Format("WindowInfo({0}, {1})", windowId, WIN_INFO_HEIGHT);
            string evalResult = InteropServices.MapInfoApplication.Eval(expr);
            return Double.Parse(evalResult);
        }

        private static Double GetWindowWidth(int windowId)
        {
            // make sure the front window is a mapper
		...
		public static int GetMapWindowId()
		{
			...
			if (windowId == 0)
			{
				MessageBox.Show(CoordsysChanger.Properties.Resources.ERR_NO_WIN_OPEN);
                return 0;
			}
			...
			if (windowType != 1)
			{
                MessageBox.Show(CoordsysChanger.Properties.Resources.ERR_FRONT_WIN_NOT_MAPPER);
				return 0;
			}
        ...
        public static MBR ReadMBRFromMITableCurrentRecord(string tableName)
        {
            ...
            if (double.TryParse(evalResult, System.Globalization.NumberStyles.Number, _usNumberFormat, out coord))
                ...
                miMBR.MinimumX = 0;
            //miMBR.MinimumX = System.Convert.ToDouble(evalResult);
            ...
            if (double.TryParse(evalResult, System.Globalization.NumberStyles.Number, _usNumberFormat, out coord))
                ...
                miMBR.MinimumY = 0;
            //miMBR.MinimumY = System.Convert.ToDouble(evalResult);
            ...
            if (double.TryParse(evalResult, System.Globalization.NumberStyles.Number, _usNumberFormat, out coord))
                ...
                miMBR.MaximumX = 0;
            //miMBR.MaximumX = System.Convert.ToDouble(evalResult);
            ...
            if (double.TryParse(evalResult, System.Globalization.NumberStyles.Number, _usNumberFormat, out coord))
                ...
                miMBR.MaximumY = 0;
            //miMBR.MaximumY = System.Convert.ToDouble(evalResult);
